  ...
  try {
    ...
    try {
      ...
      if (search_insert_error) {
        console.error(
          "[breachSearch] Database error - Failed to insert to searches table:",
          {
      ...
      if (!search_insert) {
        console.error(
          "[breachSearch] Silent error - No data returned from searches insert:",
          {
            userId,
            email: validEmail,
          }
    ...
    try {
      ...
      if (analyticsSelectError && analyticsSelectError.code !== "PGRST116") {
        console.error(
          "[breachSearch] Database error - Failed to select from analytics_cache:",
          {
      ...
      if (analyticsRow) {
        ...
        if (analyticsUpdateError) {
          console.error(
            "[breachSearch] Database error - Failed to update analytics_cache:",
            {
        ...
        const { data: analyticsInsert, error: analyticsInsertError } =
        ...
        if (analyticsInsertError) {
          console.error(
            "[breachSearch] Database error - Failed to insert to analytics_cache:",
            {
      ...
      console.error(
        "[breachSearch] Exception during analytics_cache operation:",
        {
...
router.post("/search", authMiddleware, async (req, res) => {
  ...
  try {
    ...
    try {
      ...
      if (searchTableError) {
        console.error(
          "[search] Database error - Failed to fetch user_search:",
          {
      ...
      if (!searchTableData) {
        ...
        if (insertError) {
          console.error(
            "[search] Database error - Failed to insert user_search:",
            {
              error: insertError,
              userId,
            }
    ...
    if (user_subscription === "free") {
      try {
        ...
        if (todayDate > lastSearch) {
          ...
          if (resetError) {
            console.error(
              "[search] Database error - Failed to reset search count:",
              {
                error: resetError,
                userId,
              }
        ...
        console.error(
          "[search] Exception during free subscription limit check:",
          {
    ...
    try {
      ...
      if (updateError) {
        console.error(
          "[search] Database error - Failed to update search count:",
          {
            error: updateError,
            userId,
...
router.post("/detailed-search", authMiddleware, async (req, res) => {
  ...
  try {
    ...
    try {
      ...
      if (user_id_error) {
        console.error(
          "[detailed-search] Database error - Failed to fetch user:",
          {
            error: user_id_error,
            clerkId: req.clerkId,
          }
    ...
    try {
      ...
      if (searchTableError) {
        console.error(
          "[detailed-search] Database error - Failed to fetch user_search:",
          {
            error: searchTableError,
            userId,
          }
      ...
      if (!searchTableData) {
        ...
        if (insertError) {
          console.error(
            "[detailed-search] Database error - Failed to insert user_search:",
            {
              error: insertError,
              userId,
            }
        ...
        const todayDate = new Date().toISOString().split("T")[0];
        const lastSearchDate = new Date(last_search_table.last_search_date)
          .toISOString()
          .split("T")[0];

        if (todayDate > lastSearchDate) {
          last_search_table.search_count_today = 0;
          await supabase
            .from("user_search")
            .update({
        ...
        if (
          ...
          last_search_table.search_count_today >= 10
        ) {
          return res.json({ message: "Limit exceeded for today" });
        }
    ...
    if (
      detailedData.BreachMetrics === null ||
      detailedData.ExposedBreaches === null ||
      detailedData.detail === "Not found"
    ) {
      breachedBoolean = false;
      return res.json({
    ...
    try {
      ...
      console.error(
        "[detailed-search] Exception during breach count calculation:",
        {
          error: countError,
    ...
    try {
      ...
      if (search_insert_error) {
        console.error(
          "[detailed-search] Database error - Failed to insert to searches table:",
          {
            error: search_insert_error,
            userId,
            email: validEmail,
      ...
      if (!search_insert) {
        console.error(
          "[detailed-search] Silent error - No data returned from searches insert:",
          {
            userId,
            email: validEmail,
          }
    ...
    try {
      ...
      if (analyticsSelectError && analyticsSelectError.code !== "PGRST116") {
        console.error(
          "[detailed-search] Database error - Failed to select from analytics_cache:",
          {
            error: analyticsSelectError,
            userId,
          }
      ...
      if (analyticsRow) {
        ...
        if (analyticsUpdateError) {
          console.error(
            "[detailed-search] Database error - Failed to update analytics_cache:",
            {
              error: analyticsUpdateError,
              userId,
              analyticsRow,
        ...
        if (analyticsInsertError) {
          console.error(
            "[detailed-search] Database error - Failed to insert to analytics_cache:",
            {
              error: analyticsInsertError,
              userId,
            }
      ...
      console.error(
        "[detailed-search] Exception during analytics_cache operation:",
        {
          error: analyticsError,
          message: analyticsError?.message,
          stack: analyticsError?.stack,
