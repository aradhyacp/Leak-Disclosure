...
const Monitor = ({ userPlan }) => {
  ...
  const listAnimation = useSpring({
    ...
    transform:
      monitoredEmails.length > 0 ? "translateY(0px)" : "translateY(20px)",
  ...
  const handleAddEmail = async (e) => {
    ...
    if (
      monitoredEmails.some((e) => e.email.toLowerCase() === email.toLowerCase())
    ) {
  ...
  if (userPlan !== "pro") {
    return (
      <div className="max-w-4xl">
        ...
        <div
          className={`rounded-lg border p-8 text-center ${isDark ? "bg-yellow-900/20 border-yellow-800/50" : "bg-yellow-50 border-yellow-200"}`}
        >
          <div className="text-5xl mb-4">🔒</div>
          <h3
            className={`text-xl font-semibold mb-2 ${isDark ? "text-yellow-300" : "text-yellow-900"}`}
          >
            Pro Feature
          </h3>
          <p
            className={`mb-4 ${isDark ? "text-yellow-300" : "text-yellow-800"}`}
          >
            Email monitoring is only available for Pro users. Upgrade to Pro to
            monitor your emails and get notified when they're breached.
          </p>
  ...
  return (
    <div className="max-w-6xl">
      ...
      <p className={`mb-6 ${isDark ? "text-gray-400" : "text-gray-600"}`}>
        Add email addresses to monitor. You'll be notified at your logged-in
        email when any of these addresses are found in a data breach.
      </p>
      ...
      {success && (
        <div
          className={`mb-4 p-4 rounded-lg border ${isDark ? "bg-green-900/20 border-green-800/50" : "bg-green-50 border-green-200"}`}
          role="alert"
        >
          <div className="flex items-start gap-3">
            <svg
      ...
      {monitoredEmails.length === 0 ? (
        ...
        >
          ...
          <p className={isDark ? "text-gray-400" : "text-gray-600"}>
            ...
            You'll receive notifications when any monitored email appears in a
            new breach.
          </p>
        </div>
      ) : (
        <animated.div style={listAnimation}>
          <div
            className={`rounded-lg border shadow-sm overflow-hidden ${isDark ? "bg-[#1a1a1a] border-[#2a2a2a]" : "bg-white border-gray-200"}`}
          >
            <div
              className={`px-6 py-4 border-b ${isDark ? "bg-[#0a0a0a] border-[#2a2a2a]" : "bg-gray-50 border-gray-200"}`}
            >
              <h3
