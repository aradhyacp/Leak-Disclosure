...
const SearchMail = ({ canSearch, userPlan, searchCount, onSearch }) => {
  ...
  return (
    <div className="max-w-6xl">
      <h2 className={`text-3xl font-bold mb-2 ${isDark ? 'text-white' : 'text-gray-900'}`}>Search Mail</h2>
      <p className={`mb-6 ${isDark ? 'text-gray-400' : 'text-gray-600'}`}>
        Check if an email address has been leaked in any data breaches. Get instant results.
      </p>
      ...
      {!canSearch && userPlan === 'free' && !loading && (
        <div className={`mb-4 p-4 rounded-lg border ${isDark ? 'bg-yellow-900/20 border-yellow-800/50' : 'bg-yellow-50 border-yellow-200'}`} role="alert">
          <div className="flex items-start gap-3">
            <svg className="w-5 h-5 mt-0.5 text-yellow-500" fill="currentColor" viewBox="0 0 20 20">
      ...
      {result && !loading && (
        <animated.div style={resultAnimation}>
          <div className={`p-6 rounded-lg border-2 transition-all ${
            ...
            <div className="flex items-start gap-3">
              <div className={`text-3xl flex-shrink-0 ${result.isLeaked ? 'text-red-500' : 'text-[#10b981]'}`}>
                {result.isLeaked ? '⚠️' : '✓'}
              </div>
